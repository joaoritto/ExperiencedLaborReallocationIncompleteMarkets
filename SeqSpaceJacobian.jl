...
function SeqSpaceJacobian(grids,StatEq,p)
    ...
    for w_i in 1:1#eachindex(p)
        for s in 1:1#T
            wt=[zeros(n_o,n_e) for t in 1:T]
            println("s=",s)
            for t in 1:T
                wt[t][:]=p
            end
            ...
            for o_i in 1:n_o
                for e_i in 1:n_e
                    o=(o_i-1)*n_e+e_i
                    for t in 1:T
                        Jacobian[o][w_i][s,t]=(E[t][o_i,e_i]-Ess[o_i,e_i])/dx
                    end
                end
            end
        end
...
function SeqSpaceJacobian2(grids,StatEq,p,Trss)
    ...
    for w_i in 1:length(p)
        wt=[zeros(n_o,n_e) for t in 1:T]
        for t in 1:T
            wt[t][:]=p
        end

        o_i=ceil(Int64,w_i/n_e)
        e_i=w_i-n_e*(o_i-1)

        wt[T][o_i,e_i]=p[o_i,e_i]+dx
        ...
        for t in 1:T-1
            ...
            for o_i in 1:n_o
                for e_i in 1:n_e
                    ...
                    Yoi[o][w_i][T-t+1]=((Tr*A)'*Φss-Ess[o_i,e_i])/dx
                end
            end
        end
        ...
        for ind in 1:nstates
            ...
            if e_i==1
                ...
                if e_i==1
                    ...
                    push!(i,ind)
                    push!(j,ind1_en)
                    push!(k,(1-ρ)*(1-α[e_i])*pol_σ_Ess[ind1_en])

                    push!(i,ind)
                    push!(j,ind1_un)
                    push!(k,(1-ρ)*(1-α[e_i])*(1-pol_σ_Ess[ind1_en])+ρ)

                    push!(i,ind)
                    push!(j,ind1_ep)
                    push!(k,(1-ρ)*α[e_i]*pol_σ_Ess[ind1_ep])

                    push!(i,ind)
                    push!(j,ind1_up)
                    push!(k,(1-ρ)*α[e_i]*(1-pol_σ_Ess[ind1_ep]))
                ...
                elseif e_i<n_e
                    ...
                    push!(i,ind)
                    push!(j,ind1_un)
                    push!(k,(1-ρ)*(1-α[e_i])*(1-pol_σ_Ess[ind1_en])+ρ-δ)
                ...
                elseif e_i==n_e
                    ...
                    push!(i,ind)
                    push!(j,ind1_e)
                    push!(k,(1-ρ)*pol_σ_Ess[ind1_e])

                    push!(i,ind)
                    push!(j,ind1_un)
                    push!(k,(1-ρ)*(1-pol_σ_Ess[ind1_e])+ρ-δ)
            ...
            elseif e_i==2
                ...
                if e_i==1
                    ...
                    for i1_i in 1:n_o
                        ...
                        push!(i,ind)
                        push!(j,ind1_e[i1_i])
                        push!(k,pol_σ_Uss[a1_i,i1_i]*P(θss[ind1_e[i1_i]]))

                        push!(i,ind)
                        push!(j,ind1_u[i1_i])
                        push!(k,pol_σ_Uss[a1_i,i1_i]*(1-P(θss[ind1_e[i1_i]])))
                    end
                    ...
                    for i1_i in 1:n_o
                        ...
                        push!(i,ind)
                        push!(j,ind1_e[i1_i])
                        push!(k,(1-δ-χ[e_i])*pol_σ_Uss[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-2)*n_a+a1_i,i1_i]*P(θss[ind1_e[i1_i]]))

                        push!(i,ind)
                        push!(j,ind1_u[i1_i])
                        push!(k,(1-δ-χ[e_i])*pol_σ_Uss[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-2)*n_a+a1_i,i1_i]*(1-P(θss[ind1_e[i1_i]])))
                        ...
                        if e_i==2
                            ...
                            push!(i,ind)
                            push!(j,ind1_e[i1_i])
                            push!(k,(δ+χ[e_i])*pol_σ_Uss[a1_i,i1_i]*P(θss[ind1_e[i1_i]]))

                            push!(i,ind)
                            push!(j,ind1_u[i1_i])
                            push!(k,(δ+χ[e_i])*pol_σ_Uss[a1_i,i1_i]*(1-P(θss[ind1_e[i1_i]])))
                            ...
                            push!(i,ind)
                            push!(j,ind1_e[i1_i])
                            push!(k,δ*pol_σ_Uss[a1_i,i1_i]*P(θss[ind1_e[i1_i]]))

                            push!(i,ind)
                            push!(j,ind1_u[i1_i])
                            push!(k,δ*pol_σ_Uss[a1_i,i1_i]*(1-P(θss[ind1_e[i1_i]])))
                            ...
                            push!(i,ind)
                            push!(j,ind1_e[i1_i])
                            push!(k,χ[e_i]*pol_σ_Uss[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i,i1_i]*P(θss[ind1_e[i1_i]]))

                            push!(i,ind)
                            push!(j,ind1_u[i1_i])
                            push!(k,χ[e_i]*pol_σ_Uss[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i,i1_i]*(1-P(θss[ind1_e[i1_i]])))
                        end
                    end
                end
            end
        ...
        for o_i in 1:n_o
            for e_i in 1:n_e
                ...
                A=zeros(nstates)
                A[(o_i-1)*n_e*n_a+(e_i-1)*n_a+1:(o_i-1)*n_e*n_a+e_i*n_a].=1
                Yoi[o][w_i][1]=((Tr*A)'*Φss-Ess[o_i,e_i])/dx
            end
        end
    end

    # Step 2
    P=[zeros(nstates,T-1) for o in 1:length(E)]
    for o_i in 1:n_o
        for e_i in 1:n_e
            o=(o_i-1)*n_e+e_i
            A=zeros(nstates)
