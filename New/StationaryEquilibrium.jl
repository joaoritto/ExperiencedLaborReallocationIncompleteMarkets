    ...
    for ind in 1:nstates
        ...
        if s_i==1
            ...
            if e_i==1
                ...
                push!(i,ind)
                push!(j,ind1_un)
                push!(k,(1-δ)*(1-ρ)*(1-α[e_i])*(1-pol_σ_E[ind1_en])+(1-δ)*ρ+δ)
        ...
        elseif s_i==2
            ...
            if e_i==1
                ...
                for o1_i in 1:n_o
                    ...
                    push!(i,ind)
                    push!(j,ind1_e[o1_i])
                    push!(k,(1-δ)*pol_σ_U[a1_i,o1_i]*P(θ[ind1_e[o1_i]]))

                    push!(i,ind)
                    push!(j,ind1_u[o1_i])
                    push!(k,(1-δ)*pol_σ_U[a1_i,o1_i]*(1-P(θ[ind1_e[o1_i]])))
                end
                ...
                for o1_i in 1:n_o
                    ...
                    push!(i,ind)
                    push!(j,ind1_e[o1_i])
                    push!(k,(1-δ)*(1-χ[e_i])*pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-2)*n_a+a1_i,o1_i]*P(θ[ind1_e[o1_i]]))

                    push!(i,ind)
                    push!(j,ind1_u[o1_i])
                    push!(k,(1-δ)*(1-χ[e_i])*pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-2)*n_a+a1_i,o1_i]*(1-P(θ[ind1_e[o1_i]])))
                    ...
                    if e_i==2
                        ...
                        push!(i,ind)
                        push!(j,ind1_e[o1_i])
                        push!(k,(1-δ)*χ[e_i]*pol_σ_U[a1_i,o1_i]*P(θ[ind1_e[o1_i]]))

                        push!(i,ind)
                        push!(j,ind1_u[o1_i])
                        push!(k,(1-δ)*χ[e_i]*pol_σ_U[a1_i,o1_i]*(1-P(θ[ind1_e[o1_i]])))
                        ...
                        push!(i,ind)
                        push!(j,ind1_e[o1_i])
                        push!(k,(1-δ)*χ[e_i]*pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i,o1_i]*P(θ[ind1_e[o1_i]]))

                        push!(i,ind)
                        push!(j,ind1_u[o1_i])
                        push!(k,(1-δ)*χ[e_i]*pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i,o1_i]*(1-P(θ[ind1_e[o1_i]])))
                    end
                end
                ...
                if o_i==2
                    ...
                    ind1_u=ind
                    ind1_e=[2-1,1-1,a1_i]'*[n_e*n_a,n_a,1]

                    push!(i,ind)
                    push!(j,ind1_e)
                    push!(k,(1-δ)*(1-χ[e_i])*(1-sum(pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-2)*n_a+a1_i,:]))*P(θ[ind1_e]))

                    push!(i,ind)
                    push!(j,ind1_u)
                    push!(k,(1-δ)*(1-χ[e_i])*(1-sum(pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-2)*n_a+a1_i,:]))*(1-P(θ[ind1_e])))

                    if e_i==2

                        ind1_u=n_o*n_e*n_a+a1_i
                        ind1_e=[2-1,1-1,a1_i]'*[n_e*n_a,n_a,1]

                        push!(i,ind)
                        push!(j,ind1_e)
                        push!(k,(1-δ)*χ[e_i]*(1-sum(pol_σ_U[a1_i,:]))*P(θ[ind1_e]))

                        push!(i,ind)
                        push!(j,ind1_u)
                        push!(k,(1-δ)*χ[e_i]*(1-sum(pol_σ_U[a1_i,:]))*(1-P(θ[ind1_e])))

                    else
                        ind1_u=n_o*n_e*n_a+n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i
                        ind1_e=[2-1,1-1,a1_i]'*[n_e*n_a,n_a,1]

                        push!(i,ind)
                        push!(j,ind1_e)
                        push!(k,(1-δ)*χ[e_i]*(1-sum(pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i,:]))*P(θ[ind1_e]))

                        push!(i,ind)
                        push!(j,ind1_u)
                        push!(k,(1-δ)*χ[e_i]*(1-sum(pol_σ_U[n_a+(o_i-1)*(n_e-1)*n_a+(e_i-3)*n_a+a1_i,:]))*(1-P(θ[ind1_e])))
                    end
