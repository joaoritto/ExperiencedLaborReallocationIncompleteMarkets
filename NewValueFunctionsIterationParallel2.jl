    ...
    while error>ϵ && iter<3000
        ...
        @sync @distributed for ind in eachindex(V_U)
            ...
            if e_i==1
                ...
                Veval_1(a1)=-(u((1+r)*grid_a[a_i]+ub-a1[1])+β*((1-δ-χ[e_i])*interp_W_U(a1[1])+δ*interp_W_Ud(a1[1])+χ[e_i]*interp_W_Ul(a1[1])))
        ...
        @sync @distributed for ind in eachindex(W_U)
            ...
            if e_i==1
                for i1_i in 1:n_o
                    ste=[i1_i-1,1-1,a_i]'*[n_e*n_a,n_a,1]
                    prob=P(θ[ste])
                    V_job_s[ind,i1_i]=prob*V_E[ste]+(1-prob)*V_U[ind]
                end
                ...
                for i1_i in 1:n_o
                    ...
                    ste=[i1_i-1,s1_i-1,a_i]'*[n_e*n_a,n_a,1]

                    prob=P(θ[ste])
                    V_job_s[ind,i1_i]=prob*V_E[ste]+(1-prob)*V_U[stu]
                end
