...
function Transition(grids,StatEq,zt;Guess=false,permanent=0,i_shock=1)
    ...
    if permanent==1
        ...
        (V_E0,V_U0,W_E0,W_U0,pol_a_E0,pol_a_U0,pol_μ_U0,pol_σ_E0,pol_σ_U0,J0,θ0,Φ0,Y0,E0,U0)=StatEq
    ...
    for i_i in 1:n_i
        for s_i in 1:n_s
            w0[i_i,s_i]=wages(Y0,z0[i_i],E0[i_i,:],s_i)
        end
    end
    ...
    Eplot=zeros(n_s*n_i,T)
    for t in 1:T
        for i_i in 1:n_i
            for s_i in 1:n_s
                Eplot[(i_i-1)*n_s+s_i,t]=Eold[t][i_i,s_i]
            end
        end
    end
        ...
        for t in 1:T
            ...
            for i_i in 1:n_i
                for s_i in 1:n_s
                    ...
                    wt[t][i_i,s_i]=wages(Y,z,Eold[t][i_i,:],s_i)
                end
            end
        end
